...
public class LoadConditions {
  ...
  private static final Codec<ConditionType> CONDITION_TYPE_CODEC = Identifier.CODEC.xmap(
      CONDITION_TYPES::get, ConditionType::id);
  public static final Codec<ILoadCondition> CONDITION_CODEC = CONDITION_TYPE_CODEC.dispatch(
      ILoadCondition::getType, ConditionType::codec);
  public static final MapCodecHelper<ILoadCondition> CODEC_HELPER = new MapCodecHelper<>(
      CONDITION_CODEC);
  ...
  public static <T extends ILoadCondition> ConditionType register(Identifier id,
      MapCodec<T> codec) {
    if (CONDITION_TYPES.containsKey(id)) {
      MonoLib.LOG.warn(
          "JSON Load Serializer ID {} has already been assigned to {}. Replacing with {}.", id,
          CONDITION_TYPES.get(id).codec(), codec);
    }
  ...
  public static ILoadCondition[] getConditions(JsonElement conditionData) {
    return MapCodecs.LOAD_CONDITION.getArray().decode(JsonOps.INSTANCE, conditionData).getOrThrow()
        .getFirst();
  }
